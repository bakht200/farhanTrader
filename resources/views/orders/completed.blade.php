<x-app-layout>
    ...
    <div class="bg-white rounded-lg shadow-sm p-4 mb-6">
        <form method="GET" action="{{ route('orders.completed') }}" class="flex flex-col md:flex-row md:items-center gap-4" id="search-form">
            <!-- Search -->
            <div class="flex-1">
                <div class="relative">
                    <input type="text" 
            ...
            @if(request('search') || request('category_id') !== 'all' || request('start_date') || request('end_date'))
                <a href="{{ route('orders.completed') }}" class="px-4 py-2 text-gray-600 hover:text-gray-900">
                    Clear
                </a>
            @endif
        </form>
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center space-x-3">
                                ...
                                <a href="{{ route('sales.pos.index', ['edit_order_id' => $order->id]) }}" class="text-orange-600 hover:text-orange-900" title="Edit">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                    </svg>
                                </a>
                                <button onclick="printOrderBill({{ $order->id }})" class="text-green-600 hover:text-green-900" title="Print Bill">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
            <div class="flex items-center">
                ...
                <form method="GET" action="{{ route('orders.completed') }}" class="ml-2">
                    @if(request('search'))
    ...
    <script>
        ...
        async function printOrderBill(orderId) {
            ...
            .then(data => {
                ...
                // Extract customer name from notes or customer relation
...
${(window.FTReceipt && window.FTReceipt.headerHtml) ? window.FTReceipt.headerHtml('Order Receipt') : ''}
            ...
            .catch(error => {
                ...
                // Fallback: redirect to show page and use browser print
